...
func (r resContext) Response(res interface{}) Page {
	...
	if nil != p.Config.CacheAdapter {
		...
		if cKey != "" && adapter.IsValid(cKey) {
			if cache, err := adapter.Get(cKey); err == nil {
				page.Items = res
				if err := p.Config.JSONUnmarshal([]byte(cache), &page); err == nil {
					return page
				}
			}
		}
	...
	if hasAdapter && cKey != "" {
		if cache, err := p.Config.JSONMarshal(page); err == nil {
...
func createFilters(filterParams interface{}, p *pageRequest) {
	...
	if ok {
		...
		iface := []interface{}{}
		if e := p.Config.JSONUnmarshal([]byte(s), &iface); e == nil && len(iface) > 0 {
...
func parsingNetHTTPRequest(r *http.Request, p *pageRequest) {
	...
	if strings.ToUpper(r.Method) == "POST" {
		...
		if !p.Config.CustomParamEnabled {
			...
			if err := p.Config.JSONUnmarshal(body, &postData); err == nil {
				param = &postData
			} else {
...
func parsingFastHTTPRequest(r *fasthttp.Request, p *pageRequest) {
	...
	if r.Header.IsPost() {
		...
		if !p.Config.CustomParamEnabled {
			var postData parameter
			if err := p.Config.JSONUnmarshal(b, &postData); err == nil {
				param = &postData
			} else {
				log.Println(err.Error())
			}
...
func parsingQueryString(param *parameter, p *pageRequest) {
	if i, e := strconv.Atoi(param.Size); e == nil {
		p.Size = i
	}
	...
	if i, e := strconv.Atoi(param.Page); e == nil {
		p.Page = i
	} else {
		p.Page = 0
	}
...
func valueFixer(n interface{}) interface{} {
	...
	if nil != n && reflect.TypeOf(n).Name() == "float64" {
		...
		if match, e := regexp.Match(`^[0-9]+$`, []byte(strValue)); e == nil && match {
...
func createCacheKey(cachePrefix string, pr pageRequest) string {
	key := ""
	if bte, err := pr.Config.JSONMarshal(pr); err == nil && cachePrefix != "" {
